...
**Caching**: Caches framework detection and lazy loading inventory in `.claude/cache/lazy-loading/`. Invalidated when `package.json` changes.

**Early exit**: Returns immediately if all images and components are already lazy-loaded, or if no relevant files changed.
