...
**Expected range**: 1,200–2,000 tokens (initial), 300 tokens (cache hit or early exit)

**Caching**: Caches framework detection and tool availability in `.claude/cache/accessibility/`. Invalidated when `package.json` changes.

**Early exit**: Returns immediately if the server is not accessible at the target URL, or if no relevant files changed.

**Patterns used**: Grep-before-Read, early exit, git diff scope default, template-based generation, caching, progressive disclosure
