...
<!-- upstream: Agents365-ai/drawio-skill@e740fb2898c08217952c4337b7f0c89864ce7a7e -->
...
**Expected range**: 800–2,500 tokens (initial generation), 200–500 tokens (incremental edit round)

**On-demand references**: None of the 10 reference docs (`references/`) or 28 scripts are pre-loaded — each is read only when the specific case arises (e.g. `xml-authoring.md` only when hand-writing XML, `autolayout.md` only for large graphs). This keeps the base invocation lean regardless of diagram complexity.

**Early exit**: If the draw.io CLI is missing and Python is also unavailable, returns `.drawio` XML only — skips all export and self-check steps.

**Incremental edits**: Review-loop rounds (step 6) apply targeted XML patches to the existing file rather than regenerating from scratch — 200–500 tokens per round vs. 800–2,500 for a full generation.

**Bundled generators over LLM generation**: For data-driven cases (import graphs, SQL ERD, OpenAPI, C4, sequence), Python scripts produce the XML deterministically — no token cost for geometry calculation.

**Patterns used**: On-demand references, early exit, incremental edit, template/script-based generation
